...
const { getTeamOptions, getUserOptions, getUserById } = useProjectUsers()
</script>
  ...
  <VaCard>
    ...
    <VaCardContent>
      <div v-if="projects.length > 0">
        <VaDataTable v-model:sort-by="sorting.sortBy" v-model:sorting-order="sorting.sortingOrder" :items="projects"
          ...
          <template #cell(owners)="{ rowData: project }">
            <VaAvatarGroup size="small" :options="getUserOptions(project.owners)" :max="2" />
          </template>
