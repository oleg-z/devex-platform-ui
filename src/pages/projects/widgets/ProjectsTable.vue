...
const columns = defineVaDataTableColumns([
  ...
  { label: 'Project owner', key: 'owners', sortable: false },
...
const { getUserById, getTeamOptions, getUserOptions } = inject<any>('ProjectsPage')
</script>

<template>
  <div>
    <VaDataTable v-model:sort-by="sortByVModel" v-model:sorting-order="sortingOrderVModel" :items="projects"
      ...
      <template #cell(owners)="{ rowData: project }">
        <VaAvatarGroup size="small" :options="getUserOptions(project.owners)" :max="5" />
      </template>
